    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
        <div>
          ...
          <p style={{ color: "#8b98b4", fontSize: 14 }}>Crie e gerencie seus agentes de I.A para WhatsApp</p>
        </div>
      ...
      {creating && (
        <div style={{
          ...
          <div style={{ background: "#0d1117", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 36, width: 460 }}>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button type="button" onClick={() => setCreating(false)} style={{ flex: 1, padding: "11px", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#8b98b4", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
                <button type="submit" style={{ flex: 1, padding: "11px", background: "linear-gradient(135deg, #25d366, #16a34a)", border: "none", borderRadius: 8, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Criar Agente</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: "#8b98b4" }}>Carregando agentes...</div>
      ) : agents.length === 0 ? (
        <div style={{ textAlign: "center", padding: "80px 0" }}>
          <div style={{ fontSize: 56, marginBottom: 16 }}>🤖</div>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 12 }}>Nenhum agente criado</h2>
          <p style={{ color: "#8b98b4", marginBottom: 24 }}>Crie seu primeiro agente para começar a atender clientes no WhatsApp</p>
          <button onClick={() => setCreating(true)} style={{ padding: "12px 28px", background: "linear-gradient(135deg, #25d366, #16a34a)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 20 }}>
          {agents.map(a => (
            <div key={a.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <Link href={`/app/agents/${a.id}`}>
                    <button style={{ padding: "6px 14px", background: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.25)", borderRadius: 6, color: "#25d366", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Editar</button>
                  </Link>
              ...
              <p style={{ fontSize: 13, color: "#8b98b4", marginBottom: 16 }}>{a.description || "Sem descrição"}</p>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{
                  padding: "4px 12px", borderRadius: 100, fontSize: 12, fontWeight: 700,
                  background: a.whatsapp?.status === "connected" ? "rgba(37,211,102,0.15)" :
                    a.whatsapp?.status === "qr" ? "rgba(251,191,36,0.15)" : "rgba(255,255,255,0.05)",
                  color: a.whatsapp?.status === "connected" ? "#25d366" :
                    a.whatsapp?.status === "qr" ? "#fbbf24" : "#8b98b4",
                }}>
                ...
                {a.whatsapp?.status === "connected" && a.whatsapp.phoneNumber && (
                  <span style={{ fontSize: 12, color: "#8b98b4" }}>+{a.whatsapp.phoneNumber}</span>
                )}
              </div>
            </div>
          ))}
